    ...
    public static class SocketMessageExt
    {
        public static bool ContainsRole(this SocketMessage message, int numberOfRoles = 0, SocketRole role = null)
            => role != null
                ...
                : (numberOfRoles > 0
                    ? message.MentionedRoles.Count == numberOfRoles
                    : message.MentionedRoles.Count >= 1);

        public static bool ContainsChannel(this SocketMessage message, int numberOfChannels = 0,
            SocketChannel channel = null)
            => channel != null
                ? message.MentionedChannels.Contains(channel)
                : (numberOfChannels > 0
                    ? message.MentionedChannels.Count == numberOfChannels
                    : message.MentionedChannels.Count >= 1);
